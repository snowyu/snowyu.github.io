...
最近这几个月自己也在断断续续的使用ChatGPT. 并对其有了一些感受,尽管ChatGPT也会犯错,但是总感觉它是在认真思考,虽然错的一塌糊涂.

很难想象,我知道ChatGPT(GPT3.5)是基于 Transformer 架构的语言模型,产生的回答所依赖的是从大规模的文本语料库中学习得到的统计模式和规律，从而进行下一步的预测和生成.并不是基于真正的自然语言理解和知识图谱技术。所以在它的"脑子"里没有可供查询的知识库,全是模型参数,以及预测产生下一个词汇的概率.谁也没想到这样也能推理,解决问题,甚至出现了心智能力.

这也许就是它经常搞会混淆,出错的原因.尤其是对相近的概念或名称,或者对同一项目不同版本的内容更容易混淆在一起.不过对于通识,几乎不会犯错误(出现次数足够多,权重更高).

但是,只有不停犯(尝试)错,才会有创新,是的,它已经具备初步的创造力. 比如,[协助漫画家画超现实主义漫画](https://medium.com/@chazhutton/i-asked-chatgpt-to-create-comics-then-i-drew-them-6e9622dfc30e)
...
1. 我感觉它在的阅读规范文献的首要错误在于过多的自己思考:用我以为,代替事实上
   * 经常是振振有词的,说的煞有其事
...
4. 对相近相似的专有词汇以及概念,容易混淆
   * 除非是通识(在不同的地方重复多次的,以及训练师重点调教的)
     * 不过,哪怕是训练师重点调教的,只要你有道理,能够自圆其说,也能说服AI
...
2023年3月14日, OpenAI发布了GPT4,但让人有些失望的是，这次的GPT4,并不是网传的那样继续上百倍的扩大规模(GPT3.5的500倍),而只是优化性能,局部调优.
还搞了个多模态,可以分析图像.连AI的幻觉,记忆中枢等关键核心问题都没有解决,又开辟新的识别模式,觉得神经网络层够多了?
这次对技术细节是只字不提,唯一值得一提的,可能就是可预测扩展（predictable scaling）的深度学习堆栈.

我有的两个猜测:

1. transformer神经网络存在根本缺陷,导致无法通过扩大规模来提高智能水平,这将导致不同类型的中枢在神经网络中分化出来，例如记忆中枢、语言中枢和逻辑推理中枢等。
2. OpenAI也许面临资本限制，需要从研究转为盈利。因此，他们可能会更加关注如何开发更好的商业应用，而不是单纯地追求技术突破。

最后，如果我们想要实现对人工智能的有效控制、可解释性以及审核性，我们需要将神经网络、自然语言处理和知识库三个方面结合起来，而不是仅仅使用单一的神经网络。否则，这就像是在造电子大脑一样，缺乏必要的解释和可控性。只有当我们将多个技术结合起来，才能够真正掌握和管理人工智能的发展。
...
目前还没有技术细节,只有性能测试报告,感觉不像是扩大规模(以前吹嘘的是GPT3.5的500倍),而只是对GPT3.5的进一步优化而已.
...
> 我: 五言最后一句6字.  孤舟飘荡处，为何就"人生何所征"了？神州大地上，"人群纷纷征"是什么意思? 两个征字你不觉得奇怪么? "再游心已鸣" 鸣是个啥? 诗中豪情迎, 迎啥?
...
>
> * Tag字段（1 byte）：指示这个数据包的类型，如公钥数据包、签名数据包、对称密钥数据包等等。
...
> ChatGPT:非常感谢您的纠正，我深表歉意。我的回答是错误的。根据最新的OpenPGP规范
...
> ChatGPT: 非常抱歉，我的回答存在误导。根据OpenPGP规范，确实存在Length type字段来指定Package
...
> 我: 关于 Packet Length 你还遗漏了 Partial Body Lengths 的情况
